...
# Function to convert token balances to USDT equivalent
def calculate_usdt_equivalent(balances):
    usdt_equivalents = {
        'TON': 0.2,  # Example conversion rate; adjust as needed
        'USD₮': 1,
        'CATI': 0.01,  # Example conversion rate
        'HMSTR': 0.05  # Example conversion rate
    }
    
    total_usdt = 0
    for token, balance in balances.items():
        total_usdt += balance * usdt_equivalents.get(token, 0)
